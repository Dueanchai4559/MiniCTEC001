...
export default function Menu0Page() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      if (filteredData.length === 1 && !hasShownPopup) {
        const found = filteredData[0];
        setSelectedId(found.id);
        setHasShownPopup(true);
        setSearchTerm("");

        fetch(`${baseUrlAPI}/PrescriptionById/${found.id}`)
          .then((res) => res.json())
          .then((data) => setSelectedData(data))
          .catch((err) => {
            console.error(" โหลดข้อมูลรายละเอียดไม่สำเร็จ:", err);
          });
      }
  ...
  useEffect(() => {
    // focus ตอนแรก (ถ้าไม่มี popup)
    if (!showAddPopup && !selectedId) {
  ...
  const handleAutoCreatePrescription = async () => {
    try {
      console.log("🚀 เริ่มสร้างใบสั่งยาทดสอบระบบ...");

      const response = await fetch(`${baseUrlAPI2}/prescriptions/auto-create`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });

      if (!response.ok) {
        console.warn("❌ ไม่สามารถสร้างใบสั่งยาอัตโนมัติได้");
        return;
      }

      console.log("✅ สร้างใบสั่งยาอัตโนมัติสำเร็จ");

      const createdPrescription = await response.json();

      // ✅ ดึง prescription เต็ม (พร้อม medicationHospitals)
      const detailRes = await fetch(`${baseUrlAPI}/PrescriptionById/${createdPrescription.id}`);
      const detail = await detailRes.json();

      // ✅ enrich ยาแต่ละตัวด้วยข้อมูล medication (เพื่อให้เลือกตู้ได้)
      const enrichedMeds = await Promise.all(
        (detail.medicationHospitals || []).map(async (med: any) => {
          try {
            const medRes = await fetch(`${baseUrlAPI}/medicine/byCode/${encodeURIComponent(med.medCode)}`);
            ...
            console.warn("⚠️ ไม่พบข้อมูลยา:", med.medCode);
            return med;
          }
        })
      );

      // ✅ ใส่ prescription พร้อมยาที่ enrich แล้ว ลงใน popup เลย
      setSelectedId(detail.id);
      setSelectedData({ ...detail, medicationHospitals: enrichedMeds });

      // ✅ โหลดรายการทั้งหมดใหม่ (refresh table)
      const allRes = await fetch(`${baseUrlAPI}/prescriptions`);
      const newList = await allRes.json();
      setPrescriptions(newList);
      console.log("🔄 โหลดรายการใบสั่งยาใหม่เรียบร้อย");
      ...
      if (!userId) {
        console.warn("⚠️ ไม่พบ userId → ข้ามการบันทึก log29");
        return;
      }

      const logRes = await fetch(`${baseUrlAPI}/logs29`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId,
          action: "C",
          status: "success",
          reason: "สร้างใบสั่งยาทดสอบระบบ",
        }),
      });

      if (logRes.ok) {
        console.log("📝 บันทึก logs29 สำเร็จแล้ว");
      } else {
        const err = await logRes.json();
        console.warn("❌ บันทึก logs29 ไม่สำเร็จ:", err);
      }

    } catch (error) {
      console.error("🔥 เกิดข้อผิดพลาดระหว่างสร้างใบสั่งยา:", error);
    }
  };
  ...
  return (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
      <div className="relative flex items-center gap-2 p-4">
        <h1 className="text-2xl font-bold">รายการคัดแยกใบสั่งยา</h1>
        <button
          ref={buttonRef}
      ...
      <div className="flex justify-between items-center px-2 py-2 bg-white rounded-t-md shadow-sm">
        <div className="flex justify-between items-center gap-2 px-2 py-2 bg-white rounded-t-md shadow-sm">
          ...
          <div className="flex gap-2">
            <button
              onClick={handleAutoCreatePrescription}
              className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
            >
              สร้างใบสั่งยาทดสอบระบบ
            </button>
          </div>
        </div>
        <button
